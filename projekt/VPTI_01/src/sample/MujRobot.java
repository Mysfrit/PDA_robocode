	...
	public void run() {
		...
		while (true) {
			try {
				...
				//sentence je ten string čo príde zo serveru
				var command = inFromServer.readLine();
				...
				switch (command){
					...
					case "back":
						...
						break;
					case "rotateL":
						//doubt že to bude fungovať, keď nad tým tak rozmýšlam, asi to bude fungovať na začiatok že iba choď dopredu a dozadu
						break;
					default:
						break;
				}
				...
				socket.close();
				//Nech to ten jebek posiela iba každé 2 sekundy
				Thread.sleep(2000);
